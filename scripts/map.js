...
//FT searches
  ...
  var New3High2015TableID   = "16YE6b75vMjFg7QAjyDgBKeXkBLwyTPyfUorinY1i" ;  // boundary overlay for curie, hubbard, solorio

  var SafePassage           = new google.maps.FusionTablesLayer(SafePassageTableID);
  var CHattendance          = new google.maps.FusionTablesLayer(CHattendanceTableId);
  var ESattendance          = new google.maps.FusionTablesLayer(ESattendanceTableId);
  var MSattendance          = new google.maps.FusionTablesLayer(MSattendanceTableId);
  var HSattendance          = new google.maps.FusionTablesLayer(HSattendanceTableId);
  var Networks              = new google.maps.FusionTablesLayer(NetworksTableId);
  var Community             = new google.maps.FusionTablesLayer(CommunityTableId);
  var Wards                 = new google.maps.FusionTablesLayer(WardsTableId);
  var Zipcode               = new google.maps.FusionTablesLayer(ZipcodeTableId);
  var Tiers                 = new google.maps.FusionTablesLayer(TiersTableId);
  var ILhouse               = new google.maps.FusionTablesLayer(ILhouseTableId);
  var ILsenate              = new google.maps.FusionTablesLayer(ILsenateTableId);
  var UScong                = new google.maps.FusionTablesLayer(UScongTableId);
  var New3High2015          = new google.maps.FusionTablesLayer(New3High2015TableID);

  var map;
  var geocoder;
  var addrMarker;
  var addrMarkerImage       = 'images/yellow-pin-lg.png';
  var markersArray          = [];   //for the advancedsearch marker array
  var compareArray          = [];   //for the radius marker array
  var markersArrayNetwork   = [];   //for the numbered markers in the network overlay
...
function initialize() {
  ...
  var grayStyles = [
    ...
    var chicago                 = new google.maps.LatLng(41.88, -87.68);
      var myOptions = {
        styles:                   grayStyles,
        zoom:                     11,
        center:                   chicago,
...
// ID, School, Address, City, Phone, Type, Classification, BoundaryGrades, Grades, 
// Boundary, Uniqueid, Zip, Marker, Typenum, ProgramType, Lat, Long, Rating 
...
function searchfromurl() {
  ...
  if(x === "ECP"){
    ...
    // searchtype = "advancedsearch";//early childhood program 
...
function searchInputField() {
  ...
  var theInput = $("#autocomplete").val().toUpperCase(); 
  ...
  if(theInput != "") {
    ...
    if ($.inArray(theInput, arrayforautocomplete) !== -1) {
      ...
      if(theInput.lastIndexOf("60", 0) === 0) { 
        ...
        var query = "SELECT ID, School, Address, City, Phone, Type, Classification, BoundaryGrades, Grades, Boundary, Uniqueid,"+
                          " Zip, Marker, Typenum, ProgramType, Lat, Long, Rating, "+
                          " Count, Growth, Attainment, Culture, Graduation, Mobility, Dress, Reading, Math, ACT, ADA, College "+
                          " FROM " + fusionTableId + " WHERE Zip = '" + theInput + "' ORDER BY School";
      } else {
      ...
      if(/^\d.*/.test(theInput)) {
        //check if it's a zip without a school
    ...
    // if (searchtype === "advancedsearch") { // this never happens?
    //       advancedsearch();
    //       return;
    // }
    advancedsearch();
...
function advancedsearch() {
 ...
 //console.log(query);
 _trackClickEventWithGA("Search", "Advanced Search", whereClause); 
  encodeQuery(query, resultListBuilder);
}
...
function resultListBuilder(d) {
  ...
  if( d.rows != null ) {        
    ...
    var latlngbounds = new google.maps.LatLngBounds(); // check if this should be global
    ...
    for (var i = 0; i < numRows; i++) { //start loop to build marker
      ...
      //  ID, School, Address, City, Phone, Type, Classification, BoundaryGrades, Grades, Boundary, Uniqueid,"+
      //  Zip, Marker, Typenum, ProgramType, Lat, Long, Rating"+
      //  Count, Growth, Attainment, Culture, SpecialEdCount, Mobility, Dress, PSAE, ISAT, ACT, ADA 
      //  Count, Growth, Attainment, Culture, Graduation, Mobility, Dress, Reading, Math, ACT, ADA, College
    ...
    if (searchtype === "radius") {
      resultcounthtml += ""+
      '<div class="col-xs-10" style="padding-right:3px; padding-left:10px;"><h4>' + addCommas(nlength) + ' '  + name + w + inputrad +'</h4></div>';
      ...
      resultcounthtml += ""+
      '<div class="col-xs-10" style="padding-right:3px; padding-left:10px;"><h4>' + addCommas(nlength) + ' '  + name + w + r +'</h4></div>';
    }
    ...
    if (searchtype === "address"){
      ...
      // schools with boundaries that intersect the address loc are listed
    ...
    // fits all the markers on screen and zooms out
    ...
    if ( searchtype === "radius") {
      ...
      if(filtersForDisplay.length == 0) {
        ...
        var buildresultcounthtml = '<div class="clearfix"><div class="col-xs-10" style="padding-right:3px; padding-left:10px;"><h4>0 schools found <span style="font-size:.8em;"> ['+filtersForDisplay.toString()+' ] </span></h4></div>';
...
function cleanURL() {
  // after the url and ECP search,
  // remove the ?xxxx part
